...
- 按“通道 + 语言”独立选择当前或已复核修订，支持一次导出多种译文语言的 TXT、逐图 JSON 或两者；可扁平输出、保留原训练集目录或选择性将子目录并入父级，结果可写入文件夹或流式封装为 ZIP 压缩包。
...
正常启动还会在同步依赖前扫描受限的本地开发端口范围，优先使用传统端口，
冲突时自动选择下一个可用端口，不会终止或改动占用端口的其它程序。
...
6. 在导出页为各通道和译文语言分别选择修订策略、TXT / JSON、扁平/保留/自定义目录结构、文件夹或 ZIP 输出方式和导出目录，检查阻塞错误与过期警告后开始导出；文件夹输出要求目录为空，ZIP 输出只要求同名压缩包不存在。
...
“设置 → 更新公告”记录当前源码预览版新增、改进和修复的内容。公告与代码一起提交，用户
执行 `git pull` 并重新启动后即可查看；应用不会检查远端版本、执行 Git 命令、下载更新
或安装新版本。最新公告的已读状态只保存在当前设备的浏览器本地存储中。
...
- 本项目未实现遥测或使用分析。Hugging Face 模型下载和用户主动发起的外部模型请求是
  明确的联网行为。
- 本地 API 默认只监听 IPv4 loopback；源码启动优先使用 `127.0.0.1:8765`，
  冲突时会在 `8765-8799` 中选择可用端口，不是局域网或公共服务入口。
...
源码启动器会在同步依赖前优先检查 Vite `5173` 和 API `8765`，冲突时分别在
`5173-5199` 和 `8765-8799` 中自动选择可用端口。它不会自动终止其它程序；
只有候选范围全部不可用，或用户显式指定的端口不可用时才停止并报错。此时请查看
“设置 → 关于与诊断”显示的日志目录；源码终端中的第一条错误通常最有价值。
