...
vi.mock("../../../src/features/taggers/hooks", () => ({
  useTaggerLibrary: taggerHooks.library,
  useTaggerVocabularySearch: taggerHooks.vocabulary,
}));
...
import { TagBatchEditDialog } from "../../pages/workspace/components/TagBatchEditDialog";
import { parseBatchTagDraft } from "../../../src/application/annotations/tagBatchEditState";
...
beforeEach(() => {
  ...
  taggerHooks.library.mockReturnValue({ data: { installations: [] } });
  taggerHooks.vocabulary.mockReturnValue({ data: undefined, isFetching: false });
});
...
describe("batch Tags editing", () => {
  ...
  test("ignores an older preview response after editable input changes", async () => {
    let resolveFirstPreview: ((value: typeof previewSummary) => void) | undefined;
    batchHooks.preview.mockImplementationOnce(
      () =>
        new Promise<typeof previewSummary>((resolve) => {
    ...
    const input = screen.getByRole("textbox", { name: "要添加的 Tags" });
    await user.type(input, "old_tag");
    await user.click(screen.getByRole("button", { name: "预览变更" }));
    await waitFor(() => expect(batchHooks.preview).toHaveBeenCalledTimes(1));
    await user.clear(input);
    await user.type(input, "new_tag");
    resolveFirstPreview?.(previewSummary);

    await waitFor(() => expect(screen.queryByText("预览")).toBeNull());
    await user.click(screen.getByRole("button", { name: "预览变更" }));
    await waitFor(() => expect(batchHooks.preview).toHaveBeenCalledTimes(2));
    expect(batchHooks.preview.mock.calls[1][0].request.operation.tags).toEqual([
      { name: "new_tag", category: null },
    ]);
  });

  test("clears a suggestion category when replacement text is edited manually", async () => {
    taggerHooks.library.mockReturnValue({
      data: {
        installations: [{ id: "tagger-a", fingerprint: "fingerprint-a", status: "ready" }],
      },
    });
    taggerHooks.vocabulary.mockReturnValue({
      data: {
        items: [{ name: "suggested_tag", category: "character" }],
      },
      isFetching: false,
    });
    const user = userEvent.setup();
    ...
    await user.click(screen.getByRole("button", { name: "替换" }));
    await user.type(screen.getByRole("textbox", { name: "旧 Tag" }), "old_tag");
    const replacement = screen.getByRole("textbox", { name: "新 Tag" });
    await user.type(replacement, "suggested");
    await user.click(await screen.findByRole("option", { name: /suggested_tag/ }));
    await user.clear(replacement);
    await user.type(replacement, "manual_tag");
    await user.click(screen.getByRole("button", { name: "预览变更" }));

    await waitFor(() => expect(batchHooks.preview).toHaveBeenCalledTimes(1));
    expect(batchHooks.preview.mock.calls[0][0].request.operation.replacement).toEqual({
      name: "manual_tag",
      category: null,
    });
  });

  test("keeps a raw-order detail row collapsed until its page changes", async () => {
    const user = userEvent.setup();
    render(
      <TagBatchEditDialog
        projectId="project-a"
    ...
    await user.type(screen.getByRole("textbox", { name: "要添加的 Tags" }), "new_tag");
    await user.click(screen.getByRole("button", { name: "预览变更" }));
    const detailToggle = await screen.findByRole("button", { name: /first\.png/ });
    expect(detailToggle.getAttribute("aria-expanded")).toBe("true");
    await user.click(detailToggle);
    await waitFor(() => expect(detailToggle.getAttribute("aria-expanded")).toBe("false"));
    expect(screen.queryByRole("region", { name: "修改前 Tags 顺序" })).toBeNull();
  });
});
