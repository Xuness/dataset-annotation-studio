...
import type { TagBatchEditMode } from "./tagBatchEditState";
...
export function TagBatchPreviewDetails({
  ...
  useEffect(() => {
    if (!details.items.some((item) => item.asset_id === expandedAssetId)) {
      setExpandedAssetId(details.items[0]?.asset_id ?? null);
    }
  }, [details.items, expandedAssetId]);
          ...
          {details.items.map((item) => {
            ...
            return (
              <article key={item.asset_id} className="tag-batch-dialog__detail-item">
                ...
                >
                  <img
                    src={thumbnailUrl(projectId, item.asset_id, item.content_version, 96)}
                    alt=""
                    loading="lazy"
                  />
