...
export function ProviderRequestOptionsFields({
  ...
  const isCodex = capabilities.authMode === "codex_oauth";
  ...
  return (
    <>
      <div className="provider-options-heading form-field--wide">
        ...
        <small>
          {isCodex
            ? "Codex 使用所选模型的默认推理强度；这里的设置会按对话单独覆盖。"
            : "留空的参数不会发送，交由模型或供应商使用默认值。"}
        </small>
      </div>
      {capabilities.supportsSamplingControls ? (
        <>
      ...
      {capabilities.supportsReasoningEffort ? (
        <label className={isCodex ? "form-field form-field--wide" : "form-field"}>
          <span>推理强度</span>
          <select
