  ...
  return (
    <>
      <SettingsSectionHeader
        ...
        title="本地打标器"
        description="集中管理本机模型文件与可复用打标配置；模型不会写入数据集项目。"
        actions={
          <>
            <Button
          ...
          <>
            ...
            >
              ...
              <div>
                ...
                <span>
                  {data.runtime.available ? data.runtime.providers.join(" · ") : data.runtime.error}
                </span>
              </div>
            </section>
              ...
              <section className="tagger-installation-list">
                ...
                <div>
                  {data.installations.map((installation) => (
                    <button
                      key={installation.id}
                      className={selectedInstallation?.id === installation.id ? "is-active" : ""}
                      onClick={() => {
                  ...
                  {!data.installations.length ? (
                    <p className="tagger-empty-copy">导入一个受支持的本地模型后，会在这里显示。</p>
                  ) : null}
                </div>
              </section>
                  ...
                  <>
                    <header>
                      <div>
                        ...
                        <p>
                          {selectedInstallation.adapter_name} · {selectedInstallation.model_version}
                        </p>
                      </div>
                      <span
                    ...
                    <footer>
                      ...
                      <Button
                        icon={actions.validate.isPending ? <Spinner /> : <RefreshCw size={13} />}
                        disabled={busy}
                        onClick={() =>
                          void run(
      ...
      <footer>
        <span>当前版本仅支持本地导入；不会连接或下载 Hugging Face 仓库</span>
