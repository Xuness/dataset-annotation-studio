...
- `modules/annotations`：原文保存、单项/原子批量删除、历史以及轻量校验。
...
标注批量删除由服务层一次验证整个范围，再把现有 `.txt` 暂存为同目录墓碑文件；数据库修订与状态在单个事务中
提交，任一文件或数据库步骤失败都会恢复整批。多个图片共享同一个同名 `.txt` 时，只有全部所有者都在所选范围内
才允许删除，避免批量操作影响未选图片。

素材删除位于 `modules/assets/deletions`，采用“预览计划 -> 文件恢复清单 -> 顺序移动 -> SQLite 提交”的协议：

- 删除单位是图片及其可独占的活动 `.txt`、语言译文和同名 `.json`；共享旁车保留并在预览中提示。
