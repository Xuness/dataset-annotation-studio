...
源码开发的端口选择属于启动编排边界：Node 选择器在受限的 loopback 候选范围内探测，
启动器将同一组结果传给 Vite、FastAPI 和 Tauri `devUrl`。后端不扫描进程、不终止端口占用者，
只消费最终端口配置；CORS 也只允许当次选定的前端端口。显式端口覆盖默认严格失败，无覆盖时才自动回退。

## 桌面生命周期

Windows 主窗口的关闭请求只隐藏窗口，不销毁 WebView，也不停止正在运行的任务；托盘左键和“打开 Dataset Studio”菜单会恢复并聚焦同一个窗口。托盘“退出”先唤醒窗口，再由前端统一检查未保存内容、不可中断的文件写入和可安全停止的后台任务，检查通过后才请求 Tauri 退出。Linux 不假定桌面环境一定显示托盘图标，关闭窗口会直接进入同一套安全退出检查；托盘创建失败也不会阻止应用启动。原生关闭事件携带单调递增的请求 ID，前端先确认接管该请求再执行安全检查；若页面没有加载或 WebView 无法响应，Tauri 在短暂等待后原生退出，避免不可关闭的黑色空窗口。发行版 sidecar 仍只在应用收到真正的 `RunEvent::Exit` 时终止。

Linux 源码启动器在 Tauri 开发会话外保留一层 Bash 生命周期，不用 `exec` 放弃终端所有权。
正常关窗或收到 HUP、INT、TERM 后，启动器恢复 termios、光标键与增强键盘协议，再释放当前
checkout 的会话锁并返回 Shell，避免退出日志结束后终端仍回显控制序列而无法立即重启。
...
- `modules/screening`：冻结当前任务的素材 ID，批量读取同名 Danbooru JSON，按 Rating 在批次内评分、分池，并持久化可停止/继续的结果；运行时不读取全站归档，也不改写素材。
...
缓存失效、OpenAPI 类型生成、桌面适配和状态边界详见
[新前端架构边界](frontend-v2-architecture.md)。
...
Legacy 的独立“筛选”页复用 `checkedAssetIds` 作为输入/输出选择，但每次运行都会把“全项目”、所选目录或已勾选范围解析成不可变的素材 ID 列表。`MetaRank Batch v0.1` 只在该任务可评分项内按 Rating 计算 mid-rank；依赖全库 CDF 的上下文去偏、Artist 救援和历史趋势会在能力接口中明确标为未启用。元数据错误、低证据、低分辨率、精确重复和 Danbooru 父子变体只影响结果池或附加标记，不触发文件删除、移动或改写。
...
`current` 或 `reviewed`、TXT/JSON 格式、文件夹或 ZIP 输出方式，以及扁平、保留原目录或自定义目录合并规则。默认使用当前 head；
...
目录布局默认维持兼容旧版本的扁平图片 + 同名 TXT；保留模式使用素材相对于工作区根目录的当前父路径，自定义模式
从保留结构开始，仅移除明确勾选的原始目录层级。多通道 TXT 仍先为每个通道建立独立训练集目录，再在其下应用素材
目录布局，避免把 Tags 与描述合并成含义不清的文本；逐图 JSON 可以在一个对象中保留多个通道、revision ID 和结构化 Tags。
规则目录树使用与导出计划相同的范围：项目范围走 `CandidateScope.AUTO`，显式选择范围通过素材 ID 的 POST 查询汇总，
避免大批量选择进入 URL，也避免把项目中未参与本次导出的目录显示为可配置规则。
